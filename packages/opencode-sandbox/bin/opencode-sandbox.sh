...
run_sandbox() {
	...
	# .config/zsh - mount read-write for zsh completion dumps
	if [[ -d "${HOME}/.config/zsh" ]]; then
		mkdir -p "$sandbox_home/.config/zsh"
		bwrap_args+=(--bind "/${HOME#/}/.config/zsh" "${HOME}/.config/zsh")
	fi

	# .config - mount read-only (after zsh subdir is set up)
	if [[ -d "${HOME}/.config" ]]; then
		mkdir -p "$sandbox_home/.config"
		bwrap_args+=(--ro-bind "/${HOME#/}/.config" "${HOME}/.config")
	fi

	# Create empty dircolors file in sandbox home if it doesn't exist on host
	# (prevents "dircolors: /home/vince/.dircolors: No such file or directory")
	if [[ ! -f "${HOME}/.dircolors" ]]; then
		touch "$sandbox_home/.dircolors"
		bwrap_args+=(--bind "$sandbox_home/.dircolors" "${HOME}/.dircolors")
	else
		bwrap_args+=(--ro-bind "/${HOME#/}/.dircolors" "${HOME}/.dircolors")
	fi
	...
	# Set OPENCODE_PERMISSION environment variable if yolo mode is enabled
